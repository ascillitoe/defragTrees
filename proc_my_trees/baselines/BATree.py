...
def fitBATreeCV(X, y, mdl, modeltype='regression', max_depth=[2, 3, 4], min_samples_split=[10, 20, 30], cv=5, seed=0, smear_num=100, njobs=1):
    ...
    E = np.mean(E, axis=2)
    if(mdl.score=='f1'):
        i = np.sum(E == np.max(E), axis=1).nonzero()[0][0]
        j = np.sum(E == np.max(E), axis=0).nonzero()[0][0]
    else:
...
def subfit(i, j, k, Xtr, ytr, Xte, yte, mdl, modeltype, max_depth, min_samples_split, smear_num, seed):
    ...
    if modeltype == 'regression':
        ...
        if (mdl.score=='balanced'):
            from sklearn.metrics import balanced_accuracy_score
            err = 1.0 - balanced_accuracy_score(yte,zte)
        elif (mdl.score=='standard'):
            from sklearn.metrics import accuracy_score
            err = 1.0 - accuracy_score(yte,zte)
        elif (mdl.score=='f1'):
            from sklearn.metrics import f1_score
            err = f1_score(yte,zte)
    return i, j, k, err
